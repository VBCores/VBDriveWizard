...
- подключение к приводу по USB через программатор (Serial) или по CAN (Cyphal), список найденных устройств;
...
Подтвердите установку клавишей `Y`. Менеджер установки сам подтянет все необходимые зависимости.

### 3. Разрешите доступ к USB-порту

Привод при подключении по USB виден в системе как последовательный порт (`/dev/ttyACMx`). Чтобы работать
